...
-export([code_change/4, handle_event/3, handle_info/3,
	 ...
	 idle_wait/3, init/1, negotiate/2, negotiate/3, ready/2,
	 ready/3, terminate/3, wait/2]).
...
negotiate(are_you_ready,
    ...
    notice(S,
	   "Other user ready to transfer goods:~nYou "
	   "get ~p, The other side gets ~p",
...
commit(S = #state{}) ->
    io:format("Transaction completed for ~s. Items "
	      "sent are:~n~p,~n received are:~n~p.~nThis "
