...
-export([complete/4, start_link/0]).

-export([code_change/4, dispatching/2, handle_event/3,
	 handle_info/3, handle_sync_event/4, init/1, listening/2,
	 terminate/3]).
...
handle_event({complete, Regex, Ref, Count}, State,
    ...
    NewRe = lists:keyreplace(Regex, 1, Re,
			     {Regex, OldCount + Count}),
    NewData = Data#data{regex = NewRe,
			refs = Refs -- [Ref]},
    case State of
...
dispatching({continue, File, Continuation},
	    Data = #data{regex = Re, refs = Refs}) ->
...
listening(done,
    ...
    [io:format("Regex ~s has ~p results~n", [R, C])
     || {R, C} <- Re],
    {stop, normal, done};
listening(done,
...
terminate(_Reason, _State, _Data) -> init:stop().
